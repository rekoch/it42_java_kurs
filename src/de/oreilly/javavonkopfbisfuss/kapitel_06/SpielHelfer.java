...
class SpielHelfer {
 ...
 public ArrayList<String> platziereDotCom(int dotComGrösse) {                 // Zeile 19
    ...
    String [] alphaKoordinaten = new String [dotComGrösse];      // enthält Koordinaten der Form 'f6'
	...
	ort = (int) (Math.random() * rasterGrösse);      // zufälligen Startpunkt holen
	...
	int x = 0;                                        // n-te Position in zu platzierendem DotCom
        ...
        while (erfolg && x < dotComGrösse) {                // nach angrenzenden freien Stelle suchen
          if (raster[ort] == 0) {                    // wenn noch nicht benutzt
             ...
             if (ort >= rasterGrösse){                 // ausserhalb der Grenzen - 'Ende'
               erfolg = false;                         // Fehlschlag
             }
             if (x>0 & (ort % rasterLänge == 0)) {  // ausserhalb der Grenzen - rechter Rand
               erfolg = false;                         // Fehlschlag
             }
